...
L.brw = (sc) => {
  ...
  const lbl = E('div', 'chip cy'); lbl.style.cssText += 'position:absolute;bottom:-14px;font-size:40px';
...
L.cta = (sc) => {
  ...
  card.innerHTML = `
    <div class="cta-av"><div class="ring"></div>
      <div class="in">${(sc.logo || B.logo)
        ? `<img class="logo" src="${sc.logo || B.logo}" alt="">`
        : '<i class="fa-solid fa-circle-play"></i>'}</div></div>
  ...
  return { node: s, anim(tl, t0, d) {
    if (!sc.tail) {
      ...
      if (logo) tl.fromTo(logo, { scale: .5, opacity: 0 },
        { scale: 1, opacity: 1, duration: .5, ease: 'back.out(2)' }, t0 + .3);
      tl.fromTo(tags, { opacity: 0, y: 16 }, { opacity: 1, y: 0, duration: .5 }, t0 + .8);
    }
    tl.to(ring, { rotate: 360, duration: 2.4, repeat: -1, ease: 'none' }, t0);
    tl.to(btn, { scale: 1.06, duration: .85, repeat: -1, yoyo: true, ease: 'sine.inOut' }, t0);
    if (logo) tl.to(logo, { scale: 1.05, duration: 1.6, repeat: -1, yoyo: true, ease: 'sine.inOut' }, t0 + .8);
  }};
};
...
V.tree = (s) => {
  ...
  const rows = items.map(([ic, t, dep]) => {
    const r = E('div', '');
    r.style.cssText = `display:flex;align-items:center;gap:18px;font-size:40px;font-family:var(--mono);
...
function build() {
  ...
  SCENES.forEach((sc, idx) => {
    ...
    // Vào sớm hơn mốc cảnh để chồng lên cảnh trước — hết cảm giác giật khi
    // đổi cảnh. Cắt đúng beat (`cut`, `glitch`, `flash`) thì giữ gần như tức
    // thì để còn nhịp mạnh, chỗ khác cho mềm hơn.
    const HARD = { cut: 0.06, glitch: 0.06, flashglitch: 0.06, flash: 0.06 };
    const fade = HARD[sc.out] != null ? HARD[sc.out] : 0.22;
    const tIn = Math.max(0, t0 - fade);
    tl.set(sec, { visibility: 'visible' }, tIn);
    tl.set(sec, { visibility: 'hidden' }, tEnd + 0.02);
    tl.fromTo(sec, { opacity: 0 },
      { opacity: 1, duration: fade, ease: 'power2.out' }, tIn);
